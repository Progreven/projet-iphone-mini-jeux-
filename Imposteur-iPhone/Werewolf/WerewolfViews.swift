    ...
    private var actionInstruction: String {
        ...
        if store.healingPotionAvailable && store.poisonPotionAvailable {
            if store.wolfTarget != nil {
                return "Pour sauver la victime, touchez sa case rouge. Pour empoisonner quelqu’un, touchez une autre personne."
            }
            return "Aucune victime des Loups à sauver. Touchez une personne si la Sorcière souhaite utiliser son poison."
        }
    ...
    var body: some View {
        RoleNightPanel(role: .witch, title: "Réveillez la Sorcière", instruction: actionInstruction) {
            if !store.witchIntroSeen {
                ...
                Text("Touchez de nouveau une case sélectionnée pour annuler ce choix.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .frame(maxWidth: .infinity, alignment: .leading)
