...
private struct WerewolfThiefNightView: View {
    ...
    var body: some View {
        if let revealed = store.thiefRevealPlayer {
            RoleNightPanel(
                role: .thief,
                title: "Réveillez \(revealed.name)",
                instruction: "Son rôle a été modifié par le Voleur. Montrez uniquement cette nouvelle carte à \(revealed.name)."
            ) {
                ...
                Text(revealed.role.shortDescription)
                    .foregroundStyle(.secondary)
                    .multilineTextAlignment(.center)
                    .frame(maxWidth: .infinity)

                Button(store.thiefRevealIndex == 0 ? "Passer au second joueur" : "Continuer la nuit") {
            ...
            RoleNightPanel(
                role: .thief,
                title: "Réveillez le Voleur",
                instruction: "Le Voleur choisit deux joueurs. Leurs rôles seront échangés, puis chacun découvrira son nouveau rôle séparément."
            ) {
                WerewolfActionGrid(
                    players: store.alivePlayers,
                    selectedIDs: Set(store.thiefSelectionOrder),
                    accent: .cyan,
                    maxSelection: 2,
                    onTap: { store.toggleThiefSelection($0) }
                )

                if store.thiefSelectionOrder.count == 2 {
                    Text("Les deux personnes sélectionnées recevront le rôle de l’autre.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
                ...
                Button("Valider l’échange") { store.confirmThiefSwap() }
                    .buttonStyle(PartyPrimaryButtonStyle())
                    .disabled(store.thiefSelectionOrder.count != 2)
                    .opacity(store.thiefSelectionOrder.count == 2 ? 1 : 0.45)
...
private struct WerewolfWitchNightView: View {
    ...
    private var actionInstruction: String {
        if !store.witchIntroSeen {
            return "Montrez à la Sorcière la personne visée par les Loups-Garous."
        }
        if store.healingPotionAvailable && store.poisonPotionAvailable {
            if store.wolfTarget != nil {
                return "Une seule potion peut être utilisée cette nuit. Touchez la victime rouge pour la sauver, ou une autre personne pour l’empoisonner."
            }
            return "Aucune victime des Loups à sauver. La Sorcière peut utiliser sa potion de mort sur une personne, ou ne rien faire."
        }
        if store.healingPotionAvailable {
            return store.wolfTarget == nil
                ? "La potion de mort a déjà été utilisée et il n’y a aucune victime à sauver."
                : "La potion de mort a déjà été utilisée. Touchez la victime rouge pour la sauver."
        }
        if store.poisonPotionAvailable {
            return "La potion de vie a déjà été utilisée. Touchez une autre personne pour utiliser le poison."
        }
        return "Les deux potions ont déjà été utilisées. Passez à la suite."
    }

    var body: some View {
        RoleNightPanel(role: .witch, title: "Réveillez la Sorcière", instruction: actionInstruction) {
            if !store.witchIntroSeen {
                ...
                HStack(spacing: 10) {
                    Label(
                        store.healingPotionAvailable ? "Vie disponible" : "Vie utilisée",
                        systemImage: store.healingPotionAvailable ? "heart.fill" : "heart.slash"
                    )
                    .foregroundStyle(store.healingPotionAvailable ? Color.green : Color.gray)

                    Spacer()

                    Label(
                        store.poisonPotionAvailable ? "Mort disponible" : "Mort utilisée",
                        systemImage: "drop.fill"
                    )
                    .foregroundStyle(store.poisonPotionAvailable ? Color.purple : Color.gray)
                }
                .font(.caption.bold())
                ...
                Text("Une seule potion par nuit. Touchez de nouveau la case sélectionnée pour annuler, ou choisissez l’autre action pour remplacer votre choix.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .frame(maxWidth: .infinity, alignment: .leading)

                Button("Valider le choix") { store.confirmWitch() }
                    .buttonStyle(PartyPrimaryButtonStyle())
                SkipNightButton(action: store.skipCurrentNightAction)
            }
        }
    ...
    var body: some View {
        LazyVGrid(columns: columns, spacing: 12) {
            ForEach(store.alivePlayers) { player in
                ...
                let isSaved = isWolfVictim && store.witchWillSave
                let isPoison = store.witchPoisonTargetID == player.id
                let canTap = isWolfVictim ? store.healingPotionAvailable : store.poisonPotionAvailable

                Button { store.chooseWitchAction(player.id) } label: {
                    VStack(spacing: 5) {
                        Text(player.name)
                            .font(.headline)
                            .lineLimit(1)
                        ...
                        if isSaved {
                            Label("Sauvé(e)", systemImage: "heart.circle.fill")
                                .font(.caption.bold())
                                .foregroundStyle(.green)
                        } else if isWolfVictim {
                        ...
                        if isPoison {
                            Label("Poison", systemImage: "drop.fill")
                                .font(.caption.bold())
                    ...
                    .padding(10)
                    .background(
                        isSaved ? Color.green.opacity(0.18) :
                        (isPoison ? Color.purple.opacity(0.14) :
                        (isWolfVictim ? Color.red.opacity(0.18) : Color.gray.opacity(0.10)))
                    )
                    .overlay(
                        RoundedRectangle(cornerRadius: 16, style: .continuous)
                            .stroke(
                                isSaved ? Color.green :
                                (isPoison ? Color.purple :
                                (isWolfVictim ? Color.red : Color.clear)),
                                lineWidth: (isSaved || isPoison) ? 3 : (isWolfVictim ? 2 : 1)
                            )
                    )
...
private struct WerewolfDayView: View {
    ...
    var body: some View {
        PartyPage {
            ...
            if let death = store.currentDeathAnnouncement {
                WerewolfDeathAnnouncementView(
                    death: death,
                    position: (store.deathAnnouncementIndex ?? 0) + 1,
                    total: store.deathAnnouncements.count,
                    action: store.continueDeathAnnouncement
                )
            } else if let resolution = store.pendingResolution {
                ...
                if store.needsInitialMayorElection {
                    PartyPanel {
                        Image("WWMaire")
                            .resizable()
                            .scaledToFit()
                            ...
                            .clipShape(RoundedRectangle(cornerRadius: 16, style: .continuous))
                            .frame(maxWidth: .infinity)
                        Text("Élection obligatoire du Maire")
                            .font(.title2.bold())
                            .frame(maxWidth: .infinity)
                        Text("Premier jour : le village doit élire son Maire avant de passer au débat. En cas d’égalité lors d’un vote, le Maire départagera.")
                            .foregroundStyle(.secondary)
                            .multilineTextAlignment(.center)
                            .frame(maxWidth: .infinity)
                ...
                } else if store.canUseDayVote {
                    PartyPanel {
                        Image(systemName: "person.3.fill")
                            .font(.system(size: 44))
                            .frame(maxWidth: .infinity)
                        Text("Débat et vote du village")
                            .font(.title2.bold())
                            .frame(maxWidth: .infinity)
                        Text("Laissez les joueurs débattre puis voter. Une seule élimination par vote est autorisée aujourd’hui. Après sa résolution, la nuit suivante commencera automatiquement.")
                            .foregroundStyle(.secondary)
                            .multilineTextAlignment(.center)
                            .frame(maxWidth: .infinity)
                        Button("Éliminer le joueur désigné par le vote") { showEliminate = true }
                            .buttonStyle(PartyPrimaryButtonStyle())
...
private struct WerewolfDeathAnnouncementView: View {
    let death: WerewolfDeathRecord
    let position: Int
    let total: Int
    let action: () -> Void
    ...
    private var title: String {
        switch death.cause {
        case .vote: return "Élimination du village"
        case .hunter: return "Le Chasseur a tiré"
        case .heartbreak: return "Mort de chagrin"
        case .wolves, .poison: return "Mort cette nuit"
        }
    }

    var body: some View {
        PartyPanel {
            Text(total > 1 ? "Élimination \(position) sur \(total)" : title)
                .font(.caption.bold())
                .foregroundStyle(.secondary)
                .frame(maxWidth: .infinity)

            Image(death.role.imageName)
                .resizable()
                .scaledToFit()
                .frame(maxHeight: 220)
                .clipShape(RoundedRectangle(cornerRadius: 18, style: .continuous))
            ...
            VStack(spacing: 4) {
                Text("Rôle")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                Text(death.role.title)
                    .font(.title2.bold())
            }
            .frame(maxWidth: .infinity)

            Text("Cause : \(death.cause.rawValue)")
                .font(.caption)
                .foregroundStyle(.secondary)

            Button(position < total ? "Passer à la personne suivante" : "Passer à la suite", action: action)
                .buttonStyle(PartyPrimaryButtonStyle())
        }
    }
}
