...
final class WerewolfStore: ObservableObject {
    ...
    var needsInitialMayorElection: Bool {
        phase == .day && dayNumber == 1 && !initialMayorElectionCompleted && winner == nil
    }
    var canUseDayVote: Bool {
        phase == .day && winner == nil && pendingResolution == nil && !dayVoteCompleted && !needsInitialMayorElection
    }
    ...
    func chooseWitchAction(_ id: UUID) {
        ...
        // Interface volontairement directe pour le Maître du Jeu :
        // - toucher la victime des Loups = utiliser/annuler la potion de vie ;
        // - toucher toute autre personne = utiliser/annuler la potion de mort.
