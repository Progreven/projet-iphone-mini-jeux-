...
struct HeadsUpTiltDetector {
    ...
    init(
        ...
        actionCooldown: TimeInterval = 1.0
    ) {
        ...
        self.actionCooldown = max(0, actionCooldown)
    }
    ...
    mutating func consume(z: Double, timestamp: TimeInterval) -> HeadsUpTiltAction? {
        ...
        if isLocked {
            // Après une réponse, le joueur ramène naturellement le téléphone vers
            // son front. Ce mouvement de retour peut traverser le seuil opposé.
            // On ignore donc TOUT geste pendant le cooldown, puis on exige encore
            // un vrai retour en zone neutre avant de réarmer le détecteur.
            let isNeutral = abs(rawDelta) <= neutralThreshold * 1.25 || abs(delta) <= neutralThreshold
            if isNeutral {
                neutralFrameCount += 1
            } else {
                neutralFrameCount = 0
            ...
            let cooldownFinished = timestamp >= (lockedUntil ?? timestamp)
            if cooldownFinished && neutralFrameCount >= 3 {
                isLocked = false
                lockedUntil = nil
                neutralFrameCount = 0
        ...
        if timestamp - (candidateSince ?? timestamp) >= confirmationDuration {
            isLocked = true
            lockedUntil = timestamp + actionCooldown
            neutralFrameCount = 0
            candidateAction = nil
            candidateSince = nil
            return proposed
