...
do {
    ...
    expect(store.phase == .night, "Après l'unique élimination et ses résolutions, la nuit doit recommencer automatiquement")
...
do {
    ...
    expect(store.pendingResolution == .hunter(hunter), "Le Chasseur doit agir avant la nuit")
    ...
    expect(store.phase == .night && store.nightNumber == 3, "Après le tir et sans autre résolution, la nuit doit commencer")
}
...
print("✅ Werewolf V3.1 targeted tests passed")
